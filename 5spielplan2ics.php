...
');
    ...
    for ($i=0; $i<count($expo); $i++) {
        ...
        if ($format === 'turnier') {
            ...
            $beschreibung = $spieltag . '. Spieltag';
        }
...
');
    ...
    echo '<br /><br />' . $t['output_erfolg'] . ' '
        ...
        . '<br /><br /><a href="spielplan2ics.php?lmo_sprache=' . urlencode($lmo_sprache) . '" style="display:inline-block;padding:6px 16px;background:#0d6efd;color:#fff;text-decoration:none;border-radius:4px;font-size:14px;">' . $t['btn_zurueck'] . '</a>';
}  // Ende else
echo '  </div>
</body>
</html>';
